...
## Chrome浏览器安装
1. Edge浏览器地址栏输入：`edge://extensions/`，或 Chrome 浏览器地址栏输入：`chrome://extensions/`
2. 开启右上角「开发人员模式」
...
- 适配CNKI新版、旧版和国际版界面
- 支持部分学校 WebVPN / 校外访问站域名访问的知网页面
- 自动更新，持续优化

## 校外访问说明
- 当前已适配 `webvpn-w1.gcu.edu.cn` 下的知网页面
- 若学校使用其他 WebVPN 域名，可在 `manifest.json` 和 `content.js` 中追加对应域名
- 前提是校外访问站仍保留知网主要页面结构和下载入口

---

**提示**：使用前请确保已登录 CNKI 或学校 WebVPN，并拥有对应文献的下载权限。
